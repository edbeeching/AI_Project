...
% predicate using 1111 - 10 ^ rand (0,1,2,3), but i couldnt get it working
...
%% Predicates to move columns and rows (rotation) in the Board:

%% JChong:
% If Im not wrong, Board is a List of Lists that represents a matrix, right??
...
%% but I still dont get the idea of the cut and how to used it. Reading about that on Prolog. So Ill finish Later
%%
...
%% Update, code testing. I generated the Board:
%%       1     2     3     4     5     6     7
...
get_list_of_board_connections(Player, LocationsList):- 	board(CurrentBoard),
														...
														findall(
																...
																(
																	...
																	%write('VALID MOVE: '),write(Move), nl,
...
%-------------------------------------------------------
% Game dynamics: If we speaks in terms of normal procedural languages,
% the main loop of the AI is this:
...
make_best_move(h1, Player):- !,
							...
							write(Player),write(' MOVE MADE: '),write(Move), nl,
							make_move(Move).
...
%						HEURISTIC 1 (H1)
% This heuristic evaluates the manhattan distance between the player and its target.
% This heurictic ignores the other player in its calculation entirely, it also only looks 1 ahead and tends to get stuck
%-------------------------------------------------------
...
% in a list of type[Move/Score/I/J, .....] in ListOfScores
...
h3_get_list_of_board_connections(CurrentBoard, Player, ListOfScores):-
								player(Player,_,I/J), 			% Get Players position I/J
								get_target(Player, Target),		% Get Players target position
...
h3_check_valid_move(Move):- write('INVALID MOVE: '),write(Move),nl,fail.
...
% Tries to search 2 moves ahead and evaluates the move that maximizes the score in the second move
%-------------------------------------------------------
get_target_next(Player, Target_Next):-
		treasure_list(Player,List),
		treasure_index(Player,Index),
		Index_1 is Index + 1,
		Index_1 < 6,!,
		get_element_number(Index_1,List,_Treasure/I/J).
...
h5_get_list_of_board_connections_second_move(CurrentBoard, TargetA, Next_TargetA, TargetB, ListOfVisitedNodes1, IB, JB, Score/I/J):-
											...
											findall(Sum_AC/IA/JA, (member(Move, Moves),
																	...
																	Sum_AC is (MaxScoreA + (MaxScoreC * 0.75))
															), ScoresA
													),
											...
											%write("Score A: "),write(ScoreA), nl, nl,
											h3_get_highest_score_I_J(ScoresB, ScoreB/1/1),
											h3_get_highest_score_I_J(ScoresC, ScoreC/1/1),
